 ...
 * -------------------------------------------------------------------------
 *
 * var_cast_dialect_a.c
 *
 *
 * IDENTIFICATION
 * src/common/variant/var_cast_dialect_a.c
 *
 * -------------------------------------------------------------------------
 */
#include "var_cast.h"
...
status_t var_convert_dialect_a(const nlsparams_t *nls, variant_t *var, og_type_t type, text_buf_t *buf)
{
    ...
    switch (type) {
        case OG_TYPE_UINT32:
            status = var_as_uint32_dialect_a(var);
            break;

        case OG_TYPE_INTEGER:
            status = var_as_integer_dialect_a(var);
            break;

        case OG_TYPE_BOOLEAN:
            status = var_as_bool_dialect_a(var);
            break;

        case OG_TYPE_NUMBER:
        case OG_TYPE_NUMBER3:
        case OG_TYPE_DECIMAL:
            status = var_as_number_dialect_a(var);
            break;

        case OG_TYPE_NUMBER2:
            status = var_as_number2_dialect_a(var);
            break;

        case OG_TYPE_BIGINT:
            status = var_as_bigint_dialect_a(var);
            break;

        case OG_TYPE_UINT64:
            status = var_as_ubigint_dialect_a(var);
            break;

        case OG_TYPE_REAL:
            status = var_as_real_dialect_a(var);
            break;

        case OG_TYPE_STRING:
        case OG_TYPE_CHAR:
        case OG_TYPE_VARCHAR:
            status = var_as_string_dialect_a(nls, var, buf);
            break;

        case OG_TYPE_DATE:
            status = var_as_date_dialect_a(nls, var);
            break;

        case OG_TYPE_TIMESTAMP:
        case OG_TYPE_TIMESTAMP_TZ_FAKE:
            status = var_as_timestamp_dialect_a(nls, var);
            break;

        case OG_TYPE_TIMESTAMP_LTZ:
            status = var_as_timestamp_ltz_dialect_a(nls, var);
            break;

        case OG_TYPE_TIMESTAMP_TZ:
            status = var_as_timestamp_tz_dialect_a(nls, var);
            break;

        case OG_TYPE_VARBINARY:
        case OG_TYPE_BINARY:
            status = var_as_binary_dialect_a(nls, var, buf);
            break;

        case OG_TYPE_RAW:
            status = var_as_raw_dialect_a(var, buf->str, buf->max_size);
            break;

        case OG_TYPE_INTERVAL_YM:
            status = var_as_yminterval_dialect_a(var);
            break;

        case OG_TYPE_INTERVAL_DS:
            status = var_as_dsinterval_dialect_a(var);
            break;

        case OG_TYPE_CLOB:
            status = var_as_clob_dialect_a(nls, var, buf);
            break;

        case OG_TYPE_BLOB:
            status = var_as_blob_dialect_a(var, buf);
            break;

        case OG_TYPE_IMAGE:
            status = var_as_image_dialect_a(nls, var, buf);
            break;
