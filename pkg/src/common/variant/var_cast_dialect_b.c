 ...
 * -------------------------------------------------------------------------
 *
 * var_cast_dialect_b.c
 *
 *
 * IDENTIFICATION
 * src/common/variant/var_cast_dialect_b.c
 *
 * -------------------------------------------------------------------------
 */
#include "var_cast.h"
...
status_t var_convert_dialect_b(const nlsparams_t *nls, variant_t *var, og_type_t type, text_buf_t *buf)
{
    ...
    switch (type) {
        case OG_TYPE_UINT32:
            status = var_as_uint32_dialect_b(var);
            break;

        case OG_TYPE_INTEGER:
            status = var_as_integer_dialect_b(var);
            break;

        case OG_TYPE_BOOLEAN:
            status = var_as_bool_dialect_b(var);
            break;

        case OG_TYPE_NUMBER:
        case OG_TYPE_NUMBER3:
        case OG_TYPE_DECIMAL:
            status = var_as_number_dialect_b(var);
            break;

        case OG_TYPE_NUMBER2:
            status = var_as_number2_dialect_b(var);
            break;

        case OG_TYPE_BIGINT:
            status = var_as_bigint_dialect_b(var);
            break;

        case OG_TYPE_UINT64:
            status = var_as_ubigint_dialect_b(var);
            break;

        case OG_TYPE_REAL:
            status = var_as_real_dialect_b(var);
            break;

        case OG_TYPE_STRING:
        case OG_TYPE_CHAR:
        case OG_TYPE_VARCHAR:
            status = var_as_string_dialect_b(nls, var, buf);
            break;

        case OG_TYPE_DATE:
            status = var_as_date_dialect_b(nls, var);
            break;

        case OG_TYPE_TIMESTAMP:
        case OG_TYPE_TIMESTAMP_TZ_FAKE:
            status = var_as_timestamp_dialect_b(nls, var);
            break;

        case OG_TYPE_TIMESTAMP_LTZ:
            status = var_as_timestamp_ltz_dialect_b(nls, var);
            break;

        case OG_TYPE_TIMESTAMP_TZ:
            status = var_as_timestamp_tz_dialect_b(nls, var);
            break;

        case OG_TYPE_VARBINARY:
        case OG_TYPE_BINARY:
            status = var_as_binary_dialect_b(nls, var, buf);
            break;

        case OG_TYPE_RAW:
            status = var_as_raw_dialect_b(var, buf->str, buf->max_size);
            break;

        case OG_TYPE_INTERVAL_YM:
            status = var_as_yminterval_dialect_b(var);
            break;

        case OG_TYPE_INTERVAL_DS:
            status = var_as_dsinterval_dialect_b(var);
            break;

        case OG_TYPE_CLOB:
            status = var_as_clob_dialect_b(nls, var, buf);
            break;

        case OG_TYPE_BLOB:
            status = var_as_blob_dialect_b(var, buf);
            break;

        case OG_TYPE_IMAGE:
            status = var_as_image_dialect_b(nls, var, buf);
            break;
...
status_t var_to_round_int32_dialect_b(const variant_t *var, round_mode_t rnd_mode, int32 *i32)
{
    ...
    switch (var->type) {
        case OG_TYPE_DATE: {
            ...
            cm_decode_date(var->v_date, &detail);
            int64 result = detail.year * 10000LL + detail.mon * 100LL  + (int64)detail.day;
            INT32_OVERFLOW_CHECK(result);
            *i32 = (int32)result;
            }
            break;
        case OG_TYPE_UINT32:
