...
my $num_args = @ARGV;
if ($num_args != 3) {
	print STDERR "usage: perl include.pl <search path> <base file name> <new_name_prefix>"
}

my $pg_yy_name_prefix = 'base_yy';
my $pg_core_yy_prefix = 'core_yy';
my $new_yy_name_prefix = @ARGV[2];

my $new_pg_yy_name_prefix = $new_yy_name_prefix . $pg_yy_name_prefix;
my $new_pg_core_yy_prefix = $new_yy_name_prefix . $pg_core_yy_prefix;
#
# Read from STDIN (line by line)
#---------------------------------------------------------------------
#
my $TAG= '\s*/\*\$\$include';
my $EXCLUDE_BEGIN_TAG= '\s*/\*\$\$exclude\s*in\s*dialect\s*begin\*/';
my $EXCLUDE_END_TAG= '\s*/\*\$\$exclude\s*in\s*dialect\s*end\*/';
...
my $exclude_blob_flag = 0;

while ($line = <STDIN>) {
	$line =~ s/\r//g;
	$linecnt++;

    if ($line =~ /^$EXCLUDE_BEGIN_TAG/) {
		$exclude_blob_flag = 1;
		next;
	}
    if ($exclude_blob_flag eq 1 && $line =~ /^$EXCLUDE_END_TAG/) {
		$exclude_blob_flag = 0;
		next;
	}
	if ($exclude_blob_flag eq 1) {
		next;
	}

	if ($line =~ /^$TAG\s+"(.*?)"\s*?\*\/\n/) {
		my $fname = grp($line,1);
		open(F, "< $ARGV[0]/$fname") || die("error opening input file $ARGV[0]/$fname: $!");
	...
	{
		print "%name-prefix \"$new_pg_yy_name_prefix\"\n"
	}
	elsif ($line =~ "%option prefix=\"$pg_core_yy_prefix\"")
	{
		print "%option prefix=\"$new_pg_core_yy_prefix\""
	}	
